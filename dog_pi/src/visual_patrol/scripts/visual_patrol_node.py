...
def run(img):
    ...
    center_ = []
    n = 0
    #segment the image into top, middle and bottom parts making the processing faster and more accurate.
    for r in roi:
        roi_h = roi_h_list[n]
        n += 1     
        if n <= 2:
        ...
        cnt_large, area = getAreaMaxContour(cnts)#The maximum area is found
        if cnt_large is not None:#if the contour isn't None
            ...
            #acquire the angular points of the rectangle
            ...
            #sum three centers, top, middle and bottom, according to their weight
            centroid_x_sum += center_x * r[4]
            weight_sum += r[4]

    if weight_sum is not 0:
        #acquire the center finnally obtained
...
if __name__ == '__main__':
    ...
    rospy.sleep(3)#wait for the parameters to complete loading
