...
Z_DIS = 0.2
z_dis = Z_DIS
x_pid = PID.PID(P=0.06, I=0.005, D=0)  # pid initilization
...
def getAreaMaxContour(contours):
    ...
    for c in contours:  # Traverse all the contours
        ...
        if contour_area_temp > contour_area_max:
            ...
            if contour_area_temp > 10:  # Only when the area is greater than 300, the maximum contour is effective in order to avoid interference
                area_max_contour = c
...
def run(img):
    ...
    frame_lab = cv2.cvtColor(frame_resize, cv2.COLOR_BGR2LAB)  # Convert the image into LAB space
    ...
    if __target_color in color_range:
        ...
        frame_mask = cv2.inRange(frame_lab, tuple(target_color_range['min']), tuple(target_color_range['max']))  # Perform bit operation on the original image and mask
        ...
        area_max_contour, area_max = getAreaMaxContour(contours)  # Find out the maximum contour

    if area_max > 100:  # The maximum area is found
        (center_x, center_y), radius = cv2.minEnclosingCircle(area_max_contour)  # Acquire the minimum circumscribed circle
