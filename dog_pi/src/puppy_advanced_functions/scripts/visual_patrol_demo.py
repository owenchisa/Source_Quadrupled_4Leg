...
line_centerx = -1 # coordinate of line center
img_centerx = 320
...
def run(img):
    ...
    center_ = []
    n = 0
    #Segment the image into top, medium and bottom parts, which makes the processing faster and more accurate.
    for r in roi:
        roi_h = roi_h_list[n]
        n += 1       
        if n <= 2:
            ...
            if i in __target_color:
                ...
                frame_mask = cv2.inRange(frame_lab,
                                             ...
                                             (color_range_list[detect_color]['max'][0],
                                              ...
                                              color_range_list[detect_color]['max'][2]))  #Perform bit operation on the original image and the mask                
        ...
        cnt_large, area = getAreaMaxContour(cnts)#the contour with the maximum area is found
        if cnt_large is not None:#if the contour isn't None
            ...
            cv2.drawContours(img, [box], -1, (0,0,255,255), 2)#draw the rectangle composed of four points
            
            #acquire the angular point of the rectangle
            ...
            #Sum the top, middle and bottom centers according to different weights
...
def image_callback(ros_image):
    ...
    image = np.ndarray(shape=(ros_image.height, ros_image.width, 3), dtype=np.uint8,
                       buffer=ros_image.data)  # Convert the custom image data to image
